---
title: "Referrals to mental health services"
...
We want to look at the totals for England. 
...
## Filter for referrals

Data on referrals is stored in the following measures which we will filter for:
...
```{r}
MHSDS.England.referrals <- subset(MHSDS.England, MHSDS.England$MEASURE_ID == "MHS32" | MHSDS.England$MEASURE_ID == "CCR70" | MHSDS.England$MEASURE_ID == "CCR71")

MHSDS.England.referrals <- MHSDS.England.referrals[-c(3:8)]
```

## Export data 

```{r}
write.csv(MHSDS.England.referrals, file = "MHSDS-referrals-England.csv")
```
